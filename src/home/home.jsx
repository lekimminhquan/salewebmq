import './home.scss'
import { useEffect, useState } from 'react'
import axios from 'axios'
import { replace, useLocation, useNavigate, useParams } from 'react-router-dom'
import Category from '../menu/menu.jsx'
import Product from './producShow/product.jsx'
import { productStore } from '../redux/store.jsx'
import { Categoryshow } from '../redux/action.jsx'
const Home = () => {
    const [valuesearch, setValueSearch] = useState('')
    const navigate = useNavigate()
    const [user,setUser] = useState({
        username:'',
        id:'',
        cartID:'',
    })

    const { state } = useLocation()
    useEffect(() => {
        if(state!=null){
            setUser({
                username:state.username,
                id:state.id,
                cartID:state.id
            })
        }
    },[])
    return (
        <> 
        <div className="header">
            <div className='Logominhquanhome' onClick={()=>{navigate({pathname:'/home'})}}>MQLK</div>
            <div className='headerbutton'>
                <ul>
                    <li className='combobox'><p className='searchBackground'><input type="search" placeholder='MQLK...' onChange={(e) => { setValueSearch(e.target.value) }} className='searchBox'></input> <img width="30" height="30" src="https://img.icons8.com/stickers/100/search-more.png" alt="search-more" /></p></li>
                </ul>
            </div>
            <div className='User'>
                <img width="35" height="35" src="https://img.icons8.com/pulsar-gradient/48/user.png" alt="user" />
                <a href='/login'>{user.username === '' || user.username  == null? 'Đăng Nhập/Đăng Kí':user.username }
                <div>hello</div>
                </a>
            </div>
            <div className='Cart'>
                <img src="data:image/png;base64,"></img>
                <div className='itemCart'></div>
                <a href=''>Giỏ Hàng</a>
            </div>
        </div>
        <Category></Category>
        </>
    )
}

export default Home